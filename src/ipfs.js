...
export async function startHelia(port, config = {}) {
    const blockstore = new MemoryBlockstore();
    const datastore = new MemoryDatastore();
    const libp2p = await createLibp2p({
        addresses: {
            listen: [
                path.join('/ip4/127.0.0.1/tcp/' + port.toString())
            ]
        },
        transports: [
            tcp()
...
export async function getFileFromIPFS(fs, cid) {
    const filePath = path.join('downloads/' + cid);
    try {
        const cid_ = CID.parse(cid);
        // console.log('processing cid:', cid.toString());
        const chunks = createAsyncIterableWithTimeout(fs.cat(cid_), 3000);
        let text_list = [];
